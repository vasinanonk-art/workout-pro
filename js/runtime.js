...
function uiPolishAddVersionPanel(){
 try{
  ...
  d.innerHTML='<div class="section-title">Build / QA</div><div class="msg info">Version: <b>v5.0.10</b><br>Build: Helper Injection Fix<br>QA: JS syntax / core IDs / tabs / charts preserved<br>Note: Low-risk UI only, core logic untouched</div>';
...
function modernUiHeader(){
 try{
   ...
   hero.innerHTML='<div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap"><div><div style="font-size:28px;font-weight:800">Workout PRO</div><div style="opacity:.8;margin-top:4px">Modern Hypertrophy & Recovery System</div></div><div><span class="status-pill status-good">v5.0.10</span><span class="status-pill status-warn">Modern UI</span></div></div>';
   main.prepend(hero);
 }catch(e){console.warn(e)}
}
...
function v400AddQaPanel(){
  try{
    ...
    p.id="v400QaPanel";
    p.className="card";
    p.innerHTML="<h3>v4.0 Stable QA</h3><div class='msg ok'>Helper Injection Fix<br>Router patch cleanup: ON<br>Complete card scoped to Log: ON<br>Heavy runtime patches removed: ON</div>";
    setup.appendChild(p);
  }catch(e){}
}
...
// ===== v5.0.10 Navigation Speed Cache =====
(function(){
...
// ===== v5.0.10 Alternative Button Cleanup =====
(function(){
...
// ===== v5.0.10 Alternative Action Polish =====
(function(){
...
// ===== v5.0.10 Mobile UX + Performance Shell =====
(function(){
...
// ===== v5.0.10 User Isolation Status UI =====
(function(){
...
// ===== v5.0.10 Migration Button Runtime Guard =====
(function(){
...
// ===== v5.0.10 Runtime Error Guard =====
(function(){
  if(window.__v506ErrorGuard) return;
  window.__v506ErrorGuard = true;
  window.addEventListener("error", function(e){
    try{
      console.warn("v5.0.10 guarded runtime error:", e.message);
      var mig=document.getElementById("legacyMigrationBtn");
      if(mig) mig.disabled=false;
      var save=document.getElementById("saveTeamBtn");
      if(save) save.disabled=false;
...
// ===== v5.0.10 Migration Binding Runtime Hard Fix =====
(function(){
  if(window.__v508MigrationHardBind) return;
  window.__v508MigrationHardBind = true;

  function setMigBox(msg, cls){
    var box=document.getElementById("legacyMigrationBox");
    if(box){
      box.className=cls||"msg info";
      box.innerHTML=msg;
    }
  }
  ...
  function hardBind(){
    ...
    if(btn.dataset.v508RuntimeBound==="1") return;
    btn.dataset.v508RuntimeBound="1";
    ...
    btn.addEventListener("click", function(e){
      ...
      setMigBox("กำลังเรียก Migration Function...", "msg info");
  ...
  window.addEventListener("DOMContentLoaded", function(){setTimeout(hardBind,50);});
  window.addEventListener("load", function(){setTimeout(hardBind,150);setTimeout(hardBind,1000);setTimeout(hardBind,2500);});
})();



// ===== v5.0.10 Sync Error Guard =====
(function(){
...
// ===== v5.0.10 Runtime Helper Fallback =====
(function(){
